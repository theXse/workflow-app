...
export default function RecordatoriosPage() {
    ...
    const fetchTasks = async () => {
        const { data } = await supabase
            ...
            .select("*")
            // Cambiamos el orden para que lo más nuevo (created_at desc) sea lo primero
        ...
        <div className="min-h-screen bg-black text-white p-6 md:p-12 pb-32 font-sans">
            <div className="max-w-xl mx-auto">
                <header className="mb-10 text-center md:text-left">
                    <h1 className="text-5xl font-black italic text-purple-500 mb-2 uppercase tracking-tighter">Civilia Alertas</h1>
                    <p className="text-zinc-500 text-[10px] font-black uppercase tracking-[0.3em]">Orden cronológico activo</p>
                </header>
                        ...
                        {(['Baja', 'Media', 'Alta'] as const).map((nivel) => (
                            <button
                                ...
                                className={`flex-1 py-2 rounded-xl text-[9px] font-black uppercase transition-all ${urgencia === nivel
                                        ? (nivel === 'Alta' ? 'bg-red-600 text-white' : 'bg-purple-600 text-white')
                                        : 'text-zinc-600'
                                    }`}
                            >
                                {nivel}
                    ...
                    {tasks.map((task) => {
                        ...
                        return (
                            <div key={task.id} className={`p-6 rounded-[2.5rem] border-2 transition-all flex flex-col gap-4 ${task.completada ? "bg-zinc-950 border-zinc-900 opacity-40" :
                                    task.urgencia === 'Alta' ? "bg-red-950/20 border-red-600 shadow-[0_0_15px_rgba(220,38,38,0.15)]" :
                                ...
                                <div className="flex justify-between items-center">
                                    <div className="flex-1 pr-4">
                                        <div className="flex items-center gap-2 mb-2">
                                            ...
                                            {antigua && <span className="text-[8px] font-black bg-orange-500 text-white px-2 py-0.5 rounded-full animate-pulse tracking-widest">ALERTA +24H</span>}
                                        </div>
                                        <p className={`text-lg font-bold leading-tight ${task.completada ? "line-through text-zinc-600" : "text-white"}`}>
                                            {task.tarea}
                                        </p>
                                        <p className="text-[8px] font-black text-zinc-500 uppercase mt-2">{new Date(task.created_at).toLocaleString('es-CL')}</p>
                                    </div>
                                    <div className="flex items-center gap-3">
                                        {/* BASURERO RECUPERADO */}
                                        <button onClick={() => eliminarTask(task.id)} className="text-zinc-700 hover:text-red-500 transition-colors">
                                        ...
                                        <button onClick={() => toggleTask(task.id, task.completada)} className={`w-12 h-12 rounded-full flex items-center justify-center shrink-0 transition-all ${task.completada ? "bg-zinc-800 text-zinc-500" : task.urgencia === 'Alta' ? "bg-red-600 text-white shadow-lg" : "bg-white text-black"}`}>
